...
(function ($) {
    $.fn.sortableLists = function (options) {
        ...
        function startDrag(e, el, rEl) {
            if (e.target.className && (e.target.className.indexOf('fa') !== -1 || e.target.className.indexOf('btn') !== -1))
                return;
...
function menuEditor(idSelector, settings) {
    ...
    function editItem(item) {
        ...
        $.each(data, function (p, v) {
            if ($("#mnu_" + p).is(":checkbox")) {
                
                $("#mnu_" + p).prop('checked', v == 0 ? false : true);
            } else
                $("#mnu_" + p).val(v);
        });
        $("#mnu_text").focus();
    ...
    function updateItem() {
        ...
        itemEdit.data('index', $("#mnu_index").val());
        itemEdit.data('showhome', $("#mnu_showhome").val());
        reset();
    }

    function addItem() {
        let r = Math.random().toString(36).substring(2);
        $('#frmEdit #mnu_id').val(r);
        var arrForm = $("#frmEdit").serializeArray();
        var text = $("#mnu_text").val();
        if (!text || !$('#mnu_href').val()) {
            if ($.toast)
                $.toast({
                    heading: 'Error',
                    text: 'Vui lòng nhập tên và đường dẫn danh mục',
                    showHideTransition: 'fade',
                    icon: 'error'
                })
            else
                alert('Vui lòng nhập tên và đường dẫn danh mục');
            return;
        }
        ...
        //        var iconItem = $('<i>').addClass('fas ' + $("#mnu_icon").val());
    ...
    function createMenu(arrayItem, depth) {
        ...
        $.each(arrayItem, function (k, v) {
            ...
            $li.addClass('list-group-item').
                    ...
                    data('title', v.title).
                    data('index', v.index).
                    data('showhome', v.showhome.toString());
            var $div = $('<div>');
            //            var $i = $('<i>').addClass('fas '+v.icon);
    ...
    function TButton(attr) {
        return $("<a>").addClass(attr.classCss).attr("href", "#").html(attr.text);
    }
}