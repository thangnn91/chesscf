(function ($) {
    ...
    $.fn.treeselect = function (options) {
        ...
        function createListItem(option) {
            ...
            if (option.icon)
                $label.prepend("<i class='" + option.icon + "'></i>");
            $label.prepend($input);
        ...
        function updateButtonText() {
            ...
            $div.find('input').each(function (i, el) {
                ...
                if ($checkbox.is(':checked')) {
                    ...
                    selected_val.push($checkbox.val());
                }
            ...
            if (buttontext.length > 0) {
                if (buttontext.length < 4) {
                    ...
                    $button.children('span').text('Tất cả đã được chọn');
                } else {
                    $button.children('span').text(buttontext.length + ' items được chọn');
                }
                if (settings.hiddenid) {
                    $('#' + settings.hiddenid).val(selected_val.join(','));
                }
            } else {
