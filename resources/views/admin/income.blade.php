...
<div class="m-subheader ">
    <div class="d-flex align-items-center">
        <div class="mr-auto">
            <h3 class="m-subheader__title">Danh sách khoản thu</h3>
        </div>
    </div>
</div>
        ...
        <div class="m-portlet__head">
            <div class="m-portlet__head-caption">
                <div class="m-portlet__head-title">
                    <a href="{{ route('create_income.admin') }}" class="btn btn-primary"><i class="fa fa-plus mr-2"></i> Thêm mới</a>
                </div>
            </div>
        </div>
        <div class="m-portlet__body">
            <form method="GET" action='{{ route("income.admin") }}' class="m-form m-form--fit m--margin-bottom-20">
                <div class="row m--margin-bottom-20">
                    ...
                    <div class="col-lg-3 m--margin-bottom-10-tablet-and-mobile">
                        ...
                        <select class="form-control m-select2" id="income_purpose" name="income_purpose">
                            ...
                            <option {{isset($data_back) && isset($data_back['income_purpose']) ? selected($itm->id,$data_back['income_purpose']) : ''}} value="{{$itm->id}}">{{$itm->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <table class="table table-striped- table-bordered table-hover table-checkable" id="m_table_2">
                ...
                <tbody>
                    ...
                    <tr id="{{$itm->id}}">
                        ...
                        <td>
                            <a href="{{route('edit_income.admin').'/'.$itm->id}}" class="btn btn-success btn-sm mr-2 edit_user" title="Sửa/Xem chi tiết">
...
<script type="text/javascript">
    ...
    function deleteItem(_id, _name) {
        ...
        swal({
                title: "Xóa khoản thu",
                text: "Bạn có chắc chắn muốn xóa khoản thu [" + _name + "] không?",
                type: "warning",
            ...
            .then((res) => {
                if (res.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{route('delete_income.admin')}}",
                        data: {
                            id: _id
                        },
                        success: function(data) {
                            if (data === "1") {
                                swal("Thành công!", "Xóa khoản thu thành công.", "success");
                                $('#' + _id).remove();
                            } else {
